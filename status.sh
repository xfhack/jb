...
例如本机IP为域名，输入：toyoo.pw ，如果使用本机默认IP，请空直接回车"
  ...
  别的
    ...
    读取 -erp "(默认: 127.0.0.1):" s#!/usr/bin/env bash
PATH=/bin:/sbin:/usr/bin:/usr/sbin:/usr/local/bin:/usr/local/sbin:~/bin
导出路径
...
更新壳（）{
  Set_Mirror
  sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://github.com/xfhack/jb/blob/main/status.sh" | grep 'sh_ver="' | awk -F "=" '{print $NF}' | sed 's/\"//g' | 头 -1)
  ...
  如果 [[ -e "/etc/init.d/status-server" ]] || [[ -e "/usr/lib/systemd/system/status-server.service" ]]; 然后
    ...
    Service_Server_Status_server
  菲
  wget -N --no-check-certificate "https://github.com/xfhack/jb/blob/main/status.sh" && chmod +x status.sh
